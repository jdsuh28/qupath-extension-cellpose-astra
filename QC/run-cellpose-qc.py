...
def matching(y_true, y_pred, thresh=0.5, criterion='iou', report_matches=False):
    ...
    * panoptic_quality defined as in Eq. 1 of Kirillov et al. "Panoptic Segmentation", CVPR 2019
    ...
    y_true.shape == y_pred.shape or _raise(ValueError(
        "y_true ({y_true.shape}) and y_pred ({y_pred.shape}) have different shapes".format(y_true=y_true, y_pred=y_pred)))
    criterion in matching_criteria or _raise(ValueError(
    ...
    def _single(thr):
        ...
        sum_matched_score = np.sum(
            scores[true_ind, pred_ind][match_ok]) if not_trivial else 0.0
        ...
        if bool(report_matches):
            if not_trivial:
                stats_dict.update(
                    ...
                    matched_pairs=tuple((int(map_rev_true[i]), int(
                        map_rev_pred[j])) for i, j in zip(1+true_ind, 1+pred_ind)),
...
def matching_dataset(y_true, y_pred, thresh=0.5, criterion='iou', by_image=False, show_progress=True, parallel=False):
    ...
    """
    len(y_true) == len(y_pred) or _raise(ValueError(
...
def matching_dataset_lazy(y_gen, thresh=0.5, criterion='iou', by_image=False, show_progress=True, parallel=False):
    ...
    expected_keys = set(('fp', 'tp', 'fn', 'precision', 'recall', 'accuracy', 'f1', 'criterion',
                        'thresh', 'n_true', 'n_pred', 'mean_true_score', 'mean_matched_score', 'panoptic_quality'))
    ...
    if parallel:
        ...
        def fn(pair): return matching(*pair, thresh=thresh,
                                      criterion=criterion, report_matches=False)
        with ThreadPoolExecutor() as pool:
            stats_all = tuple(pool.map(fn, tqdm(y_gen, **tqdm_kwargs)))
    else:
        stats_all = tuple(
            matching(y_t, y_p, thresh=thresh,
                     criterion=criterion, report_matches=False)
    ...
    accumulate = tuple(namedtuple('DatasetMatching', acc.keys())
                       (*acc.values()) for acc in accumulate)
...
def compareLabels(model_name, image_folder, out_dir=None):
    ...
    # Grab all tif images
    raw_images = [x for x in Path(image_folder).glob(
        "*.tif") if not ("masks" in x.name or "flows" in x.name)]

    # Define deterministic output directory
    if out_dir is not None and str(out_dir).strip() != '':
        results_path = Path(out_dir)
        results_path.mkdir(parents=True, exist_ok=True)
        out_csv = results_path / "qc_results.csv"
        if out_csv.exists():
            raise FileExistsError(
                f"Refusing to overwrite existing QC results: {out_csv}")
        file_path = out_csv
    else:
        ...
        file_path = results_path / \
            ("Quality_Control for " + model_name + ".csv")

    with open(file_path, "w", newline='') as file:
        writer = csv.writer(file, delimiter=",")

        writer.writerow(["model", "image", "Prediction v. GT Intersection over Union", "false positive", "true positive", "false negative",
                        "precision", "recall", "accuracy", "f1 score", "n_true", "n_pred", "mean_true_score", "mean_matched_score", "panoptic_quality"])
    ...
    # define the images
        for raw in raw_images:
            ...
            writer.writerow([model_name, raw.name, str(iou_score), str(stats.fp), str(stats.tp), str(stats.fn), str(stats.precision), str(stats.recall), str(
                stats.accuracy), str(stats.f1), str(stats.n_true), str(stats.n_pred), str(stats.mean_true_score), str(stats.mean_matched_score), str(stats.panoptic_quality)])
