...
public class CellposeBuilder {
     ...
     *
     * ASTRA:
     * Mirrors {@link #modelDirectory(File)} so QC output location can be controlled explicitly
     * by the ASTRA pipeline rather than inferred later.
     */
    ...
    public Cellpose2D build() {
        ...
        // ASTRA: resolve deterministic QC and results directories via AstraHooks.
        File qcDirResolved = AstraHooks.resolveQcDirectory(quPathProjectDir, this.qcDirectory);
        this.qcDirectory = qcDirResolved;
        if (!qcDirResolved.exists())
            qcDirResolved.mkdirs();

        File resultsDirResolved = AstraHooks.resolveResultsDirectory(quPathProjectDir);
        if (!resultsDirResolved.exists())
            resultsDirResolved.mkdirs();

        cellpose.outputModelName = this.outputModelName;
        cellpose.modelDirectory = this.modelDirectory;
        cellpose.qcDirectory = this.qcDirectory; // ASTRA: explicit QC directory propagated into Cellpose2D.
        cellpose.resultsDirectory = resultsDirResolved; // ASTRA: deterministic results directory propagated into Cellpose2D.
